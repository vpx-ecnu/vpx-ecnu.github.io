...
def load_sources(max_publications: int, fetch_detail_pages: bool):
    ...
    return scholar_publications, selected_publications
...
def write_output(publications: List[Dict]) -> Path:
    output_path = repo_root() / "public" / "publications" / "publication_updated.json"
...
def build_publication_updated(max_publications: int = 200, fetch_detail_pages: bool = True) -> Dict[str, int]:
    scholar_publications, selected_publications = load_sources(
        max_publications=max_publications,
        fetch_detail_pages=fetch_detail_pages,
    )
